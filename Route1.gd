...
export var player_position = Vector2(495, 425)
...
func _on_CaveOfWondersEntrance_body_entered(body):
	if (body.name == "Player"):
		emit_signal("map_transition", "CaveOfWonders", Vector2(80, 488))
		
func handle_encounter():
	var pokemon = Utils.generateEncounter(available_pokemon)
	emit_signal("begin_encounter", pokemon, "Route1", $Player.position)
